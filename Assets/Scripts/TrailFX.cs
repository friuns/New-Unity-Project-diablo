...
public class TrailFX : MonoBehaviour 
{
	...
	void Start ()
	{
        pl = transform.root.GetComponent<Shared>();
	    if (pl == null)
	    {
	        Destroy(gameObject);
	        return;
	...
	{
        //bool b = pl.isAttacking && pl.info.normalizedTime > .3f && pl.info.normalizedTime < .7f;
        //b = false;
        if (pl == null) return;
	    bool b = false;
        Vector3 pos = transform.position - pl.pos;
        float magnitude = (pos - oldpos).magnitude / Time.deltaTime;
        //print(magnitude);
	    if (!pl.alive)
	        enabled = false;
        if (magnitude > 15 && pl.isAttacking)
            b = true;
        oldpos = pos;
        renderer.enabled = b;
        if (b)
